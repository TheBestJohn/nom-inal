...
| **Recipe sharing** | Private by default; share one and everyone can read and log it — every account, and anyone holding its link at `/r/{slug}` without signing in. That link is a server-rendered page, so it unfurls in a chat app as the recipe rather than as this paragraph. Only you can change it, and un-sharing takes the link away. Unlike foods, a recipe is yours |
...
| `PUBLIC_ORIGIN` | _empty_ | The address this instance is reached at, e.g. `https://nom.example`. Only a shared recipe's page needs it, for absolute `og:` and canonical URLs. Empty means "take it from the request", which is right behind the bundled nginx. |
...
**A shared link is a page, not an app.** `/r/{slug}` is served by the API as a
finished HTML document, and nginx proxies it there rather than handing over the
SPA shell. Everything that makes a shared link useful depends on that: Slack,
iMessage, Discord, Facebook, Twitter and Google fetch the URL, read the
`<head>`, and stop — none of them runs the script that would tell them which
recipe it is, so a client-rendered page previews every shared recipe
identically. Serving the document also means the reader, who usually has no
account and is on a phone, gets the ingredients without downloading an
application to read them; that it prints, works with JavaScript off and carries
schema.org `Recipe` data all follow from the same decision. The structured data
is the same shape the importer reads off other people's sites, so a recipe
shared from one instance imports into another, and a test asserts the
round-trip rather than assuming it.

**A recipe's link says what it is, and old links do not rot.** The address is a
slug made from the name — `/r/pierogi-ruskie` — generated by one function that
both create and rename call. Renaming issues a new slug and keeps the old one
in `recipe_slug_history`, whose primary key is the slug itself: a link somebody
sent last year still resolves, is redirected once to the current address, and
can never be handed to a different recipe. Uuids resolve the same way, so every
link shared before slugs existed still works. One canonical address per recipe,
arrived at from any of them.

**Every shared recipe has a picture.** `og:image` is not optional in practice —
a preview without one is a grey box in most clients. A recipe with photos gets
its first photo re-encoded to 1200×630 and cropped to cover; one without gets a
card drawn from its name and figures on the theme's green, with the font
vendored into the binary because the runtime image has none. Both are tagged
with an `ETag` built from the recipe's `updated_at` and its photo set and
answer `If-None-Match` with a 304, because a link posted in a busy channel is
fetched once per client that renders it.
...
GET    /public/recipes/{id}              # a shared recipe by slug or uuid, no token
GET    /public/recipes/{id}/preview.png  # its 1200x630 link-preview card
GET    /public/photos/{id}               # its photos, and nowhere else without one

GET    /r/{slug}                         # NOT under /api/v1: the shared recipe as a
                                          # finished HTML page, which is what a chat app
                                          # or a search engine reads. An old slug or a
                                          # uuid 301s to the current one
